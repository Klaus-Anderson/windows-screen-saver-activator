...
def create_icon_hidden_window():
    ...
    icon_path = "lattice.ico"
    icon_flags = win32con.LR_LOADFROMFILE | win32con.LR_DEFAULTSIZE
    hicon = win32gui.LoadImage(wc.hInstance, icon_path, win32con.IMAGE_ICON, 0, 0, icon_flags)
    ...
    win32gui.SetMenuDefaultItem(menu, 1023, 0)
    ...
    win32gui.Shell_NotifyIcon(win32gui.NIM_ADD,
                              (hwnd, 0, win32gui.NIF_ICON | win32gui.NIF_MESSAGE | win32gui.NIF_TIP,
                               win32con.WM_USER + 20, hicon, "Idle Time"))
    return hwnd
...
def wnd_proc(hwnd, msg, wparam, lparam):
    if msg == win32con.WM_COMMAND:
        ...
        win32gui.TrackPopupMenu(win32gui.GetSubMenu(win32gui.GetMenu(hwnd), 0), win32con.TPM_RIGHTBUTTON,
                                win32api.LOWORD(lparam), win32api.HIWORD(lparam), 0, hwnd, None)
    return win32gui.DefWindowProc(hwnd, msg, wparam, lparam)


if __name__ == '__main__':
    hidden_window = create_icon_hidden_window()
    if idle_time(15): # Check if the elapsed time since the last input is greater than 15 seconds
        # Remove the icon from the taskbar
        win32gui.Shell_NotifyIcon(win32gui.NIM_DELETE, (hidden_window, 0))
        win32gui.PostQuitMessage(0)  # Quit the message loop
        print("The mouse and keyboard have been idle for 15 seconds.")
